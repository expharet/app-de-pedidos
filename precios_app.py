...
def render_dashboard():
    ...
    if _nuevos:
        ...
        for _np in sorted(_nuevos, key=lambda x: x.get('fecha',''), reverse=True)[:5]:
            ...
            st.markdown(f'<div style="{_col_alerta}">📦 <b>{_np_id}</b> — {_np_cliente} — <b>${_np_total:,.2f} USD</b> — {_np_dest} — <small style="color:#667085">{_np_fecha}</small></div>', unsafe_allow_html=True)
        if len(_nuevos) > 5:
...
def build_order_html(ped):
    ...
    for item in ped.get('productos',[]):
        ...
        rows+=(f'<tr><td style="padding:7px;border:1px solid #e0e0e0">{cod}</td>'
               ...
               f'<td style="padding:7px;border:1px solid #e0e0e0;text-align:right;color:#667085">${flete_u:.4f}</td>'
...
def send_order_email(ped):
    ...
    html = (f'<html><body style="font-family:Arial,sans-serif;color:#333">'
            ...
            f'{_notas_html}'
            f'</div><p style="color:#667085;font-size:11px">Export Haret \u00a9 2026 | order@exportharet.com</p>'
            f'</body></html>')
...
def render_portal_pedido():
    ...
    if not _step2_ok:
        ...
        st.markdown(f'<div style="text-align:center;color:#667085;margin-top:28px"><small>{LANG_TEXTS[st.session_state.get("portal_lang","es")]["footer_text"]}</small></div>', unsafe_allow_html=True)
        return
        ...
        if _grp_agg:
            ...
            for _gk, _gv in sorted(_grp_agg.items()):
                ...
                _grp_html.append(
                    ...
                    f'<br><small style="color:#667085">{_g_breakdown}</small>'
                    f'</div>'
                    f'</div>'
                )
    ...
    for idx, p in enumerate(prods):
        ...
        if _ux_base_price_1:
            ...
            if precio_u < _ux_base_price_1:  # YA con descuento por volumen
                _ux_strike_html = (f'<span style="color:#6b7280;text-decoration:line-through;'
    ...
    if st.session_state.portal_carrito:
        ...
        for _ci, _item in enumerate(st.session_state.portal_carrito):
            ...
            if tipo_precio == 'CIF':
                ...
                if _ux_base_price and _ux_curr_price and _ux_base_price > _ux_curr_price:
                    ...
                    _ux_save_html = f'<span class="eh-meta-cell" style="color:#6b7280;text-decoration:line-through">${_ux_base_price:.2f}/cj</span><span class="eh-meta-cell" style="color:#10a37a;font-weight:700">{_T["savings_per_box"].format(d=_ux_diff)}</span>'
            _resumen_html.append(f'''
        ...
        st.markdown(
            '<div style="margin:6px 0;padding:5px 0"><small style="color:#667085">' + _T['rate_info'].format(sym=_disp_sym, rate=_disp_rate, m=_disp_mon, live=_live_lbl_t, src=_rate_src, ts=_rate_ts) + '</small><br><small style="color:#667085"><i>' + _T['rate_info_sub'].format(m=_disp_mon) + '</i></small></div>',
            unsafe_allow_html=True
        )
        if tipo_precio == 'FOB':
    ...
    if st.session_state.portal_carrito:
        ...
        with st.expander(_T['calc_title'], expanded=False):
            ...
            <small style='color:#667085;display:block;margin-top:8px'>{_T['calc_note']}</small>
            </div>""", unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="text-align:center;color:#667085"><small>{LANG_TEXTS[st.session_state.get("portal_lang","es")]["footer_text"]}</small></div>', unsafe_allow_html=True)
...
def main():
    ...
    st.markdown('---')
    st.markdown('<div style="text-align:center;color:#667085;"><small>🚀 Export Haret © 2026 | Sistema Profesional de Gestión de Pedidos</small></div>',unsafe_allow_html=True)
